...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        for i in range(math.ceil(num_frames / args.gop)):
            tenFirst = np.array(Image.open(os.path.join(args.input , 'im' + str(i * args.gop + 1) + '.png')).resize((448, 256))).astype(np.float32) * (1.0 / 255.0)
            ...
            for batch in range(2, batch_range):
                tenSecond = np.array(Image.open(os.path.join(args.input , 'im' + str(batch) + '.png')).resize((448, 256))).astype(np.float32) * (1.0 / 255.0)
