...
if __name__ == "__main__":
    ...
    w, h, _ = (256, 448, 3)#np.array(Image.open(os.path.join(args.input , "im1.png"))).shape
    ...
    compflow, side_compflow, cfx_shape, cfy_shape, cfz_shape, compres, side_compres, rex_shape, rey_shape, rez_shape, clipped_recon_image = testnet.compress(testtfprvs, testtfnext)
    
    flowtensors = [compflow, side_compflow, cfx_shape, cfy_shape, cfz_shape]
    restensors = [compres, side_compres, rex_shape, rey_shape, rez_shape]
    ...
    with tf.Session() as sess:
        ...
        for i in range(math.ceil(num_frames / args.frequency)):
            imFirst = Image.open(os.path.join(args.input , 'im' + str(i * args.frequency + 1) + '.png')).resize((448, 256))
            tenFirst = np.array(imFirst).astype(
            ...
            for batch in range(2, batch_range):
                imSecond = Image.open(os.path.join(args.input , 'im' + str(i * args.frequency + batch) + '.png')).resize((448, 256))
                tenSecond = np.array(imSecond).astype(
                    np.float32) * (1.0 / 255.0)
