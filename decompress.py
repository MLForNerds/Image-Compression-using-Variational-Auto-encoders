...
if __name__ == "__main__":
    ...
    side_compflow = tf.placeholder(tf.string, [1], name="compressed_of_side_string")
    ...
    cfz_shape = tf.placeholder(tf.int32, [2], name="compressed_of_lengthz")

    compres = tf.placeholder(tf.string, [1], name="compressed_residue_string")
    side_compres = tf.placeholder(tf.string, [1], name="compressed_residue_side_string")
    ...
    rez_shape = tf.placeholder(tf.int32, [2], name="compressed_residue_lengthz")

    testnet(testtfprvs, testtfprvs) #required to call call() to call build()

    recimage = testnet.decompress(testtfprvs, compflow, side_compflow, cfx_shape, cfy_shape, cfz_shape, compres, side_compres, rex_shape, rey_shape, rez_shape)
